...
namespace FSEditor.MapDescriptor
{
    public interface ST7_Interface
    {
        ...
        bool isHackExtendedVentureCardTable(EndianBinaryReader stream, Func<UInt32, int> toFileAddress);
        void writeHackExtendedMapDescriptionTable(EndianBinaryWriter stream, Func<UInt32, int> toFileAddress);
        bool isHackExtendedMapDescriptionTable(EndianBinaryReader stream, Func<UInt32, int> toFileAddress);
