...
{
    public class DesignTypeTable : DolIOTable
    {
        protected VAVAddr writeTable(List<MapDescriptor> mapDescriptors)
        {
            var designTypeTable = new List<UInt32>();
            foreach (var mapDescriptor in mapDescriptors)
            {
                designTypeTable.Add((UInt32)mapDescriptor.Theme);
            }
            return allocate(designTypeTable, "DesignTypeTable");
        }
        protected override void writeAsm(EndianBinaryWriter stream, AddressMapper addressMapper, List<MapDescriptor> mapDescriptors)
        {
            ...
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca38), SeekOrigin.Begin); stream.Write(PowerPcAsm.mulli(0, 3, 0x04));
            // r3 <- 0x80428e50 ->  r3 <- tableAddr
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca3c), SeekOrigin.Begin); stream.Write(PowerPcAsm.lis(3, v.upper16Bit)); stream.Write(PowerPcAsm.addi(3, 3, v.lower16Bit));
            // lwz r3,0x14(r3)   ->  lwz r3,0x0(r3)
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca48), SeekOrigin.Begin); stream.Write(PowerPcAsm.lwz(3, 0x0, 3));
        }
        ...
        {
            if (isVanilla) {
                s.Seek(0x14, SeekOrigin.Current); // offset
            }
            foreach (var mapDescriptor in mapDescriptors)
            {
                mapDescriptor.Theme = (BoardTheme)s.ReadUInt32();
                if (isVanilla)
                {
        ...
        {
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca3c), SeekOrigin.Begin);
        ...
        {
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca38), SeekOrigin.Begin);
