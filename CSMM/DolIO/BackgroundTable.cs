...
{
    public class BackgroundTable : DolIOTable
    {
        protected VAVAddr writeTable(List<MapDescriptor> mapDescriptors)
        {
            var backgroundTable = new List<VAVAddr>();
            foreach (var mapDescriptor in mapDescriptors)
            {
                backgroundTable.Add(allocate(mapDescriptor.Background));
            }
            return allocate(backgroundTable, "BackgroundTable");
        }
        protected override void writeAsm(EndianBinaryWriter stream, AddressMapper addressMapper, List<MapDescriptor> mapDescriptors)
        {
            ...
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca80), SeekOrigin.Begin); stream.Write(PowerPcAsm.mulli(0, 3, 0x04));
            // r3 <- 0x80428e50 ->  r3 <- tableAddr
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca84), SeekOrigin.Begin); stream.Write(PowerPcAsm.lis(3, v.upper16Bit)); stream.Write(PowerPcAsm.addi(3, 3, v.lower16Bit));
            // lwz r3,0xc(r3)   ->  lwz r3,0x0(r3)
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca90), SeekOrigin.Begin); stream.Write(PowerPcAsm.lwz(3, 0x0, 3));
        }
        ...
        protected override void readAsm(EndianBinaryReader s, List<MapDescriptor> mapDescriptors, AddressMapper addressMapper, bool isVanilla)
        {
            ...
            foreach (var mapDescriptor in mapDescriptors)
            {
                mapDescriptor.Background = resolveAddressToString((VAVAddr) s.ReadUInt32(), s, addressMapper);
                if (isVanilla)
                {
        ...
        {
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca84), SeekOrigin.Begin);
        ...
        {
            stream.Seek(addressMapper.toFileAddress((BSVAddr)0x801cca80), SeekOrigin.Begin);
